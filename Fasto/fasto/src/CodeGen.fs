  ...
  | Write(e, tp, pos) ->
    ...
    match tp with
      ...
      | Array elem_type -> (* for Array(Int) and Array(Array(Int)) *)
           ...
           let header2 = [ mipsLoad elem_size (tmp_reg, arr_reg, 0)
           ...
           (* create a fake Fasto node corresponding to the array elem
              ...
              element *)
           let elem_var  = newName "ft_elem_var"
           let new_vtab  = SymTab.bind elem_var tmp_reg vtable
           let fastoexp : TypedExp = Write(Var(elem_var, pos), elem_type, pos)
